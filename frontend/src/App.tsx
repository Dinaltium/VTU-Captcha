...
function App() {
  ...
  const fetchExams = async () => {
    if (loadingExams) return; // Prevent multiple calls
  ...
  const handleFetchResults = async () => {
    ...
    try {
      // Send everything in one request - backend will handle navigation
      const response = await fetch(`${API_BASE_URL}/fetch-results`, {
        ...
        body: JSON.stringify({
          ...
          exam_type: selectedType,  // Send type instead of branch_url
          fetch_details: true,
        }),
      });
